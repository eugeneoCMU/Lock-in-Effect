...
@pytest.mark.parametrize("wrong", ["263", "341", "424"])
def test_wrong_abstract_word_count_fails(tex, letter_file, wrong):
    """Every one of these was the true count at some point this week, which is
    precisely why a stated count has to be recomputed rather than trusted."""
    letter_file.write_text(
        letter_file.read_text().replace("taken it to 328 words", f"taken it to {wrong} words", 1))
    ok, info = letter_check(tex)
...
def test_word_count_claim_deleted_fails(tex, letter_file):
    letter_file.write_text(
        letter_file.read_text().replace("taken it to 328 words", "lengthened it", 1))
    ok, info = letter_check(tex)
