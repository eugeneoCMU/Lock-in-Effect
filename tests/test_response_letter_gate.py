...
@pytest.mark.parametrize("wrong", ["263", "328", "352", "359", "366", "367", "280", "424", "226", "235", "248", "287", "294", "323"])
def test_wrong_abstract_word_count_fails(tex, letter_file, wrong):
    ...
    letter_file.write_text(
        letter_file.read_text().replace("taken it to 341 words", f"taken it to {wrong} words", 1))
    ok, info = letter_check(tex)
...
def test_word_count_claim_deleted_fails(tex, letter_file):
    letter_file.write_text(
        letter_file.read_text().replace("taken it to 341 words", "lengthened it", 1))
    ok, info = letter_check(tex)
