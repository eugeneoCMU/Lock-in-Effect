...
def test_the_printed_cr1_rungs_are_the_artifact_rounded():
    ...
    assert f"${c1[0]:+.1f}$ to ${c1[1]:+.1f}$" == "$+3.1$ to $+8.3$"
    assert f"${c1b[0]:+.1f}$ to ${c1b[1]:+.1f}$" == "$+2.7$ to $+8.8$"
...
def test_the_conventional_rung_is_the_committed_run_bit_for_bit():
    ...
    FP2-B1: the invariant object is the FLOOR-UNIT interval, which must be
    bit-identical to v1's; the pp image re-derives under the extended grid
    (v1's lower-edge floor 5.702% sits in the refit's (5, 6) interval), so
    the pp comparison is a bounded-movement check, not bit equality."""
    v1 = json.loads((ROOT / "hazard" / "data"
                     / "floor_inference_correction_results.json").read_text())
    v1r = v1["reads"][FLOOR_LADDER_READ]["cr1_t_interval"]
    assert v1r["floor_ci95_pct"] == RD["cr1_t_interval"]["floor_ci95_pct"]
    for a, b in zip(v1r["marginal_ci95_pp"],
                    RD["cr1_t_interval"]["marginal_ci95_pp"]):
        assert abs(a - b) < 0.0902  # the extension's gated movement bound
...
def test_the_designer_units_clause_is_the_webb_interval_in_floor_units():
    ...
    assert abs(w["marginal_ci95_pp"][0] - 2.8252003470251736) < 1e-9
