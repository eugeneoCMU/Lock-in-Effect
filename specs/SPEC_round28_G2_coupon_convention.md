...
## POST-RUN AMENDMENTS G2A-AM2 + G2B-AM1 (labeled, 2026-07-29 — first executions stopped on their own gates)

**G2A-AM2.** The φ_id replay gates targeted bit-exact reproduction of committed cells that date from an earlier round's SOMA/FRED vintage; realized identity-leg deviations are input-vintage drift (≤$0.0025B, ≤0.004pp share, 7e-6 r — 3–6 orders below the −1.9pp finding). Gates re-anchored to drift-aware tolerances ($0.005B / 0.005pp / 1e-4), with the seam itself proven inert bit-exactly by G3b (bucket identity at φ_id) and G3c (marginal invariance, bit-exact at both floors, realized 70.3450604199/42.6083938811). All converted deltas are same-run vs φ_id. C1 splits: monotone ordering stays blocking (PASSED); the projection bracket becomes a recorded EXPECTATION MISS — realized composed 98.113% vs projected 98.737% (band edge 98.573): the linear one-bucket projection under-predicts because the Δ=0.80 split moves 60% of each bucket two grid steps into the SOMA book's 2.0%-coupon mass (36.1% weight). The miss is reported in the verdicts table, not absorbed.

**G2B-AM1.** (a) The ABM-series reconstruction failed its own FREE replay gate (H0b[abm] against figures/theil_data.json) and H2 (flat reconstruction +0.1055pp vs 0.05 tolerance) → per the script's own pre-committed fallback the ABM companion is NOT QUOTED; hazard basis only; those gates become the fallback record (non-blocking). (b) Pa ("levels rise everywhere") and Pb ("additive constant", r-threshold) as drafted were IMPOSSIBLE given WP-H1's committed clip finding: four doubly-clipped months force wedge = 0 and carry the whole span, and the clipped-series correlation moves (0.0043–0.0065) are entirely the clip's shape effect (an additive constant cannot move a demeaned correlation). Amended blocking forms: rise-everywhere-except-doubly-clipped (wedge == 0 exactly there), additive invariance EXACT (1e-9) on the unclipped diagnostic series, r-invariance EXACT (1e-9) on the unclipped diagnostic. The drafted forms and the clipped-series movements are retained in the artifact as non-blocking records (ORIGINAL_DRAFT_*/CLIP_SHAPE_*). The branch classifier adjudicates peaks and U2 on the reported series and additivity/r on the unclipped diagnostic; BOTH adjudications land in the verdicts row, and the branch flip (ii)→(i), if realized, is disclosed as an amendment consequence, never silent.

**G2B-AM1 second-pass correction (labeled).** AM1(b)'s "additive constant is exact on the unclipped diagnostic" was itself wrong: two scheduled-amortization curves diverge with loan age, so the unclipped wedge is a deterministic DRIFT (realized span 0.009–0.014pp), not a constant. The corrected blocking form asserts the true identity — the unclipped wedge equals sched(2.5%)−sched(leg) ELEMENTWISE (1e-10) — and thresholds the drift's correlation effect at the drafted R_MAX_MOVE=0.005 (realized ~0.001). Nothing else changes.
