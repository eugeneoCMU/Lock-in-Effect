...
| C-72 | MAJOR | 3 | RUN | **RUN DONE, LANDING QUEUED** | run `fdf2ef3` Branch A: G_month=6 measured; month rung t(5) +3.091..+8.421 (vs stratum +3.153..+8.350); two-way CGM computable, df 5, +2.281..+9.339. E3 PASS, E4 MISSED (no grid truncation). tab:ladder rows not yet added | RUN: add a month-clustered rung and a two-way (stratum x month) rung t |
| C-73 | MAJOR | 3 | RUN | **SATISFIED** | 2ae04e3 | RUN (analytic): recompute the implied cross-sectional gradient at the |
| C-74 | MAJOR | 3 | RUN | **LANDED (PARTIAL) — one site reserved for Eugene** | run `225a5e9`, landed at §V.B + TECHNICAL + `tab:runindex`, GATE #116, 75-test battery. Bracket a in {0, 1/r=0.8144, 1, r=1.2279}; in-sample 7.3332/8.8522/9.1985/9.6236pp, off-window 4.4434/5.3622/5.5716/5.8287pp. E3 PASS but deliberately not advertised (linear in declared inputs). Rise above 0.797x stated as FORCED. **OPEN: the `tab:assembly` row at .tex:374 is NOT edited** — adding the attenuated member is an assembly disposition and the M3 off-window cell (+5.83) sits above the binding interval's midpoint, so its placement (counterweights vs downward ladder) is Eugene's call, not the coordinator's | RUN: score the Ginnie leg with the elasticity attenuated by the measur |
...
| C-79 | MAJOR | 3 | RUN | **RUN DONE, LANDING QUEUED** | run `3d45e69` Branch A: break-even IO share 0.3080 first order / 0.3427 second; at the source-verified 45% the gap is -$19.1531bn -- REVERSES under FACE accounting. E3/E4 PASS. [posture] + new bib entry needed; .tex not yet edited | RUN: score a Danish leg with an interest-only share |
