...
# C2 AMENDMENTS (labeled, PRE-RUN, 2026-07-29 — C1 executed after this spec was drafted; nothing below is discretionary at run time)

- **A1 (parity re-anchor).** P2's target is C1's REALIZED Webb wild-*t* interval from `floor_inference_correction_v2_results.json` `reads.R2…wild_t_webb.marginal_ci95_pp` = `[2.8549950653913494, 8.677971792194077]` (width `5.822976726802727`), not the pre-C1 predicted `[2.796…, 8.723…]` (width 5.9268) quoted in C2.3. Reproduction inputs pinned: `R_smm = 0.004257129719866917`, `se_cr1_smm = 0.0003053670536820312`, t* CSV `floor_inference_v2_tstar_R2_webb.csv` (9,999 rows, column `t_star`), edge-truncation convention as in C1.
- **A2 (expectation restate, realized widths).** `w₂/w₁ = 2.2935/5.8230 = 0.3939` → RSS `+7.5%` wider. Asymmetric about `+5.5716`: lower `√(2.7166² + 1.0098²) = 2.898`, upper `√(3.1064² + 1.2837²) = 3.361` → **predicted convolved ≈ `[+2.67, +8.93]`pp, width ≈ 6.26pp**. Comonotone bound `5.8230 + 2.2935 = 8.117`pp. **Verdict rule UNCHANGED**: convolved width must exceed 5.8230pp or STOP.
- **A3 (P4 anchors).** `ANCHORS_BY_FLOOR` extended with the committed band-6.5 cells of `oos_identification_results.json .instrument1_marginal_table`: `4.695 → (785.7883567116477, 734.0068506865199, 6.771052540089784)`; `5.334 → (743.8451864084004, 711.2235461485112, 4.265670450690081)` (the 4.991 row of the same table bit-matches the existing anchors — same provenance).
- **A4 (landing anchors post-C1).** The manuscript now prints `$+2.9$ to $+8.7$`; every C2.4/C2.7 reference to the old printed pair maps to the current pair (plus the one surviving labeled Rademacher secondary at the ladder). The C2.4 draft sentences are re-anchored to current text at apply time with count-asserted replacements as usual.
- **A5 (percentile replicates).** C1's v2 run persisted only the two t* CSVs, NOT the 1,000 percentile floor draws → C2 uses C2.0's standalone fallback: regenerate `fu.cluster_bootstrap_cpr` on the R2 selection under rng 42 with draws dumped, **gated on** `se_pp = 0.3933971547430653` and `ci95_pct = [4.368576777908708, 6.003552252675821]` (bit tolerances 1e-9); on miss → STOP, land nothing.
- **A6 (P4 execution).** The interaction probe RUNS (not declined): `bootstrap_pathb_cluster.py --floor 4.695 --reps 60` and `--floor 5.334 --reps 60`, floor-tagged outputs, tolerance as C2.5 (5–95 width within ±25% of the 4.991 width).
- **A7 (P2 split — the base spec's per-draw 1e-9 is mathematically unattainable; labeled PRE-RUN, 2026-07-29).** C1's committed Webb interval is computed by the QUANTILE ROUTE (percentile the t* draws, then map); the per-draw route (map every draw, then percentile) differs by ~½|g″|w(1−w)h² ≈ 1e-7pp because np.percentile's linear interpolation between order statistics does not commute with the curved PCHIP map. P2 therefore splits: **P2a** quantile route, tol 1e-12, PLUS a t*-CSV identity assertion against the committed `.wild_t_webb.t_star_q` (proves the CSV is C1's draw set); **P2b** per-draw route, tol 1e-6pp PLUS the tolerance-free assertion that the per-draw percentile and C1's value lie in the SAME adjacent-order-statistic bracket. Both blocking. Also adopted: **P0** (the A5 regeneration is its own blocking gate, schema superset), the single FRED MORTGAGE30US fetch inherited from `mdr.build_panel()` (C1's own committed panel path — not an engine or macro-model call), P3's zero-deviation read as the degenerate one-point loan layer under the EXCLUSION convention with a 1e-12 cross-check against `fu._map_draws`, and P4's blocking-only-on-integrity semantics (tolerance routes the landing per C2.5). Drafting agent's derivation independently verified by the coordinator before this amendment was committed.

# C6 AMENDMENTS (labeled, PRE-RUN, 2026-07-29 — drafting surfaced spec-vs-code discrepancies; coordinator verified each at source level before adopting)

- **A1 (the ddof fork — spec-literal P2/P3/G1 at 1e-9 are unreachable).** Production spec v4 was fit by `seasonality_concave_gap.fit_with_month_dummies`, whose standardizer runs on `.to_numpy()` arrays (numpy `.std()`, ddof=0; `seasonality_concave_gap.py:84,91-93` — coordinator-verified); `bootstrap_se.fit_betas` standardizes with pandas `.std()` (ddof=1; `bootstrap_se.py:108-111`). The gap/friction coefficients differ by √(n/(n−1)) ≈ 3.2e-5 at n=10,176 — four orders outside 1e-9. BLOCKING FORMS ADOPTED: C6(i) P2b = verbatim `fit_with_month_dummies` replay at 1e-9 (P2a/P2c reported); P3a = frozen-artifact replay blocks, P3b refit-gap diagnostic; C6(iii) primary leg = the production ddof-0 design (G1 achievable there), with the spec-literal `fit_betas` leg reported alongside.
- **A2 (v3 leg pins `coef_path` explicitly; month effects travel via a temporary artifact** — `simulate_qt_window` reads `month_effects` from `coef_path` whose default is now the v4 artifact, and `predict_hazard` carries no month terms).
- **A3 (cold-fit evidence + convexity reading).** The committed v4 `.fit_method` string is the COLD branch's; warm-branch strings differ — G1 compares alpha and reports both strings. The penalized objective is strictly convex, so C6(iii) probes CONVERGENCE, not multimodality (`convexity_note` fixed in the artifact); `BETTER_OPTIMUM_FOUND` remains STOP. The `prod_direct` leg (IRLS prestep bypassed — the literal dispersed-start probe) RUNS in addition to the default legs.
- **A4 (fixed ex ante by the draft, adopted):** seeds 1000+s, s=0..47, rung=s//12; σ floor max(rung·|head|, 0.05); v4 "materially higher" failure rate ≥10/200; verdict precedence C6(i) GRID_IDENTITY_FAILS > MOVES > SURVIVES, C6(ii) NARROWS > V4_UNSTABLE > SURVIVES_*; P1 comparator = every numeric/bool/string leaf at 1e-12 with prose `note` leaves skipped-and-counted; tier-2 FE level-shift disclosed and tier 2 enters no manuscript number; all-12-months training-window assertion added. Cost note: run `--limit` timing probes before the full sequence.
- **A5 (scope split of the four "not recomputed at adoption" sites; labeled, post-C6(i)-run).** The (i-a) landing rule listed 4 occurrences, but two of them (the spec-v3 decomposition sentence and the resampling-distribution sentence) describe objects `pathA_ridge_v4` does NOT recompute — they are C6(ii)'s scope and land with `pathA_resimulate_joint`. C6(i) deletes the phrase only at the two ridge-verification sites. Also: the v4 reference-swap sentence is worded as consistent with the WARM-START policy, not as evidence the cold-start sensitivity is gone (v3's committed 1.2892 was a cold-start artifact; the v4 leg warm-starts per this spec's own engine entry).
- **A6 (C6(iii) anchor-as-evaluation; coordinator signs the flagged gate change; labeled, PRE-RUN).** The anchor is the frozen 317-vector EVALUATED on the leg's design (exact unit translation b→b·s_design/s_ddof0 on the two pandas-standardized macro columns; burnout ddof-0 in both constructions, factor 1), never a refit — an evaluation cannot fail to converge. G1's BLOCKING clause becomes column_map_exact AND finite evaluation AND alpha match; the warm-start-refit-reproduces-committed-macros check is RETAINED verbatim but non-blocking (the production fit's standing |grad|≈0.014 warning is C6(iii)'s premise, not a failure). Convergence policy = the committed pipeline's own (finite params AND max|macro| ≤ MAX_ABS_BETA, mirroring pathA_resimulate_joint.py:337); optimizer warnings diagnostic-only. Cross-leg anchor spread expected ~1e-8 (penalty-only difference 2e-9), recorded.
- **A7 (C6(ii) Stage B cannot be adjudicated yet; labeled, POST-RUN).** Stage B v3 realized verdict NARROWS (joint [689.0, 1217.6]B, width ratio 0.094) — but 106/198 draws sit at EXACTLY the accounting ceiling 1217.6339B (= Σ|QT targets| when simulated prepayment ≈ 0), and the saturated draws are the rogue-optimizer-mode replicates (mean rate_gap +2.23, friction +0.67 sign-flipped; the committed hazard_bootstrap_draws.csv itself has rate_gap mean 1.49 — the committed population is bimodal, and the rogue mode is the optimizer-path artifact the paper documents at the reference-swap site). The committed 3-beta rendering of the same rogue draws produced the −$4460B tail; the joint rendering saturates the ceiling instead: two renderings of one pathology, and neither tail is a sampling statement about the estimand. REQUIRED BEFORE ANY LANDING: (1) the aggregation KeyError at pathA_resimulate_joint.py:887 fixed (specs-subset runs must not index spec3 unconditionally); (2) a NEW BLOCKING Stage-B parity leg — the PRODUCTION head pushed through stage_b's exact coefficient handoff must reproduce the committed 915.067027857209 B (tol $0.50B) for v3; (3) Stage B results reported under BOTH decompositions, fixed here: saturated-vs-not (mechanical) and production-branch-vs-rogue (all three macros within 0.02 of production, C6(iii)'s own branch rule); (4) the verdict is then re-adjudicated on the pre-committed codes with the ceiling saturation disclosed — the provisional NARROWS is NOT accepted; if the point-head parity fails, the whole Stage B construction is void (wiring, not finding).
- **A5 trail closure (C6(ii) landed).** Of the two resampling-scope "not recomputed at adoption" sites: the resampling-distribution sentence is REPLACED by the measured joint-rerun statement (committed interval stays quoted, ii-b); the DECOMPOSITION sentence is RETAINED AS CORRECT — pathA_resimulate_joint does not recompute the window-thirds decomposition of the spec-v3 point, so its disclaimer stands.
