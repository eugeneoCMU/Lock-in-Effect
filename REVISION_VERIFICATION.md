...
## Round 12 addendum (2026-07-11, resubmission notice; round-11 embed defect found and fixed)

1. **Round-11 embed never landed on disk.** The panel's round-12 notice reported that the round-10 letter had arrived verbatim a second time (so the round-11 letter was never delivered) and quoted `v15_parity_report_round10.json` at `ba26e2…` per the round-10 manifest. Filesystem check confirmed a defect on this side: the bundle's parity report was still in round-10 state — the corroboration of Round 11 item 1 had been recorded in the letter and in this document but never written into the artifact, and the round-11 letter's own inlined manifest carried the old `ba26e2…` hash, contradicting its claim that the hash had moved.
2. **Fix applied 2026-07-11, before any delivery.** The arithmetic was independently re-verified against the rendered `revised_paper_v15r5.docx` (exactly 37 non-empty paragraphs after the References heading; + heading = 38 units = 135 − 97), then the corroboration was appended to `round8_to_round9_drop_note` — the sole field changed; both direction tallies intact (27 and 98; the embedded 27 + 98 = 125 assertion unaffected). `SHA256SUMS.txt` regenerated (41 entries, exactly one change), the round-11 letter's inlined manifest line corrected to match, `v15r5_bundle.zip` re-cut (43 files). New hash: `078f0caeb8c2f8c2190d690d4e9cdf41bb2c2649a09875a87acc30a2575e23b0`. Round 11 item 1 is true of the artifact only as of this fix; the round-10 letter was left untouched as a historical record.
3. **Lesson (standing):** "claims updated, artifact untouched" is the same defect class the parity check polices in the manuscript editions. After writing any claim about an artifact's content or hash — in a letter, in this document, or in session memory — re-hash the artifact in the same session and diff it against the claim before cutting the bundle.
