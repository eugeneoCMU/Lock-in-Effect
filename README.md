...
The two core models are linked: the ABM precomputes a 3D behavioral **CPR (Conditional Prepayment Rate) surface** over `(rate × friction × rate_velocity)`, and the macro model interpolates that surface month-by-month to build counterfactual "what the Fed's roll-off would have looked like under each institutional regime" scenarios — including **scheduled amortization** and **income-scaled curtailment** (partial voluntary prepayments), so the comparison is apples-to-apples with reality.
...
- **ABM explains about half the gap**: the full model (behavioral ABM + dynamic friction + scheduled amortization + curtailment) predicts **$344.6B** of U.S. trapped liquidity — **51.2%** of the empirical figure. The remaining ~49% likely reflects pool-composition, vintage/seasoning, and servicer effects outside the household decision function.
- **Behavioral extensions widened, not closed, the gap**: adding three literature-grounded mechanisms (43% DTI hard wall, Kahneman-Tversky 2.25x loss aversion, 20% rate-shock freeze) reduced the ABM's trapped-liquidity estimate from the pre-behavioral $544.0B (80.8%) to $419.6B (62.3%). See [TECHNICAL.md §15](TECHNICAL.md).
- **Curtailment also worsened the match**: partial voluntary prepayments scaled on real disposable income (FRED `DSPIC96`, ~1.2% CPR at full scale) added **$74.9B** of simulated roll-off during QT, pushing the headline figure to $344.6B (51.2%) — confirming the pre-registered expectation that an additive prepayment channel cannot close a gap caused by the ABM already over-predicting mobility. See [TECHNICAL.md §16](TECHNICAL.md).
- **The institutional gap**: under the dynamic-balance Danish counterfactual, the portfolio would have **overshot** the QT cap by **-$779.0B** (shrinking from $2,634B to $455B), versus the U.S. system's $344.6B shortfall. The resulting **institutional gap is $1,123.6B**.
- **Monthly CPR fit is weak, but not for the reason you'd expect**: the ABM's CPR tracks the empirical *level* reasonably well but has a **negative correlation with the month-to-month path** (raw r = -0.31, smoothed r = -0.45 — smoothing makes it *worse*). A lag-correlation check (±3 months) found no lag that flips this meaningfully positive, so the mismatch is a genuine structural gap (vintage effects, seasoning, path dependence) rather than settlement-timing noise.
- **Out-of-sample validation**: splitting the sample at January 2024, the ABM performs **better** out-of-sample (R² = -0.675, share explained 54.5%) than in-sample (R² = -3.562, share explained 48.0%) — evidence against pure curve-fitting, since a curve-fit model would be expected to degrade, not improve, out of sample.
- **Robustness**: the empirical benchmark is stable across data sources (SOMA vs. WSHOMCB differ by <1%) and moves in a reasonable, expected range ($285B-$690B) across 18 different QT cap-schedule assumptions. The ABM's own trapped-liquidity estimate is stable across 50 independent population draws (95% CI: $357.3B-$370.7B).
