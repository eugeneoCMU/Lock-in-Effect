...
wild-cluster small-sample correction at $+2.9$ to $+8.7$ points (its lower edge since re-read as $+2.8$ under the extended map; run
...
labeled second line, $[+2.80, +8.99]$ points (since superseded by $[+1.71, +9.38]$ on the restricted rung): wider than either input, offered
...
estimation-panel and bootstrap tables have moved to the appendix verbatim; the one
diagnostic table sits in the Path~A estimation-detail appendix. No cell, note or caveat was edited --- in
...
The suite has grown since the point-by-point above; the current counts are read off
the repository's gate suite (\texttt{tools/liveness\_gates.py}) and test suite rather
than quoted here, because a count quoted in a letter is a number with nothing checking it.
