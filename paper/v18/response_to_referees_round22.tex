...
the purpose --- and nothing was de-hedged. The abstract went to 263 words. Three
deliberate additions have since taken it to 359 words: returning the agent-based model to
it, which the cut had orphaned against a section the model still occupies,
reporting the lock-in contribution as a range rather than as a point (\S7), and
...
wild-cluster-corrected $+2.8$ to $+8.7$ interval on 31 clusters, with the
narrower percentile read named and marked as under-covering. I would
rather give you the trail than the endpoint, since the endpoint is longer than 263
...
record here what has changed. Four of the seven items below move \emph{against} the
...
involuntary floor is not a term in its move rule at all but a \emph{product} of it,
...
wild-cluster small-sample correction at $+2.8$ to $+8.7$ points (run
...
the headline sits just below the corrected interval's midpoint while every correction
falls in its lower half; and $+5.6$ is therefore a middle member of a range whose
measured corrections concentrate below it. I publish no composed point, because the
one joint cell this paper has run for two adjustments returned an interaction of
$-1.47$ points.

Two things keep that from being a retraction, and both are in the same paragraph. The
interval does not contain zero, and the sign is fixed by the window's rate
configuration rather than by any of these calibrations. And the corrections that are
not floor corrections run the other way: the additive form returns $+11.2$ points at
the same anchors, the Fonseca--Liu (2024) anchor would put the marginal at $+11.5$
...
involuntary floor in 68.8\% of evaluated loan-months, against 36.3\% at the in-sample
